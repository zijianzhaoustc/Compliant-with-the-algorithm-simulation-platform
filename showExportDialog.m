...
%   用户可独立选择设置参数、直方图、计算结果、窗口扫描和两路时间戳；
%   时间戳仍采用
%   每行一个数值、无表头格式，便于再次导入平台。

parentPos=parent.Position;
dialogFigure=uifigure('Name','选择导出内容','WindowStyle','modal', ...
    'Position',[parentPos(1)+parentPos(3)/2-220 parentPos(2)+parentPos(4)/2-210 440 420]);
g=uigridlayout(dialogFigure,[11 2]); g.ColumnWidth={145,'1x'};
g.RowHeight={30,30,30,30,30,30,30,30,30,36,24}; g.Padding=[12 12 12 12];
...
timestamps=uicheckbox(g,'Text','Start/Stop 时间戳 TXT','Value',false); timestamps.Layout.Row=5; timestamps.Layout.Column=[1 2];

l=uilabel(g,'Text','时间戳/直方图单位'); l.Layout.Row=6; l.Layout.Column=1;
unit=uidropdown(g,'Items',{'ps','ns','s'},'ItemsData',{1e-12,1e-9,1},'Value',1e-12); unit.Layout.Row=6; unit.Layout.Column=2;
l=uilabel(g,'Text','文件名前缀'); l.Layout.Row=7; l.Layout.Column=1;
base=uieditfield(g,'text','Value','coincidence_result'); base.Layout.Row=7; base.Layout.Column=2;
l=uilabel(g,'Text','导出目录'); l.Layout.Row=8; l.Layout.Column=1;
folder=uieditfield(g,'text','Value',pwd); folder.Layout.Row=8; folder.Layout.Column=2;
browse=uibutton(g,'Text','选择目录…','ButtonPushedFcn',@browseFolder); browse.Layout.Row=9; browse.Layout.Column=[1 2];
exportButton=uibutton(g,'Text','开始导出','FontWeight','bold','BackgroundColor',[.16 .52 .35], ...
    'FontColor','white','ButtonPushedFcn',@doExport); exportButton.Layout.Row=10; exportButton.Layout.Column=[1 2];
status=uilabel(g,'Text','请选择内容和保存位置'); status.Layout.Row=11; status.Layout.Column=[1 2];
    ...
    function doExport(~,~)
        ...
        options=struct('settings',settings.Value,'histogram',histogram.Value, ...
            ...
            'timestamps',timestamps.Value,'unitSeconds',unit.Value);
        if ~any([options.settings options.histogram options.results options.sweep options.timestamps])
