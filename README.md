...
- 偶然符合：理论公式、边带、循环时间平移
- 计数指标：`RA`、`RB`、`Rraw`、`Racc`、`Rnet`、`Rtrue`、偶然符合占比
- 时间指标：峰位置、RMS 标准差、FWHM、峰位置误差
...
实测 TXT 不包含 `pairID` 和光子对生成率，因此 `Rtrue`、峰位置误差、Precision、Recall、F1、TP/FP/FN、窗口捕获率和符合估计双路联合效率显示 `N/A`。实测数据的条件效率先用设置的暗计数率和背景计数率修正两路单计数，再计算 `A|B=Rnet/RBcorr`、`B|A=Rnet/RAcorr`。
